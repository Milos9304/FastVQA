...
TEST(ansatz_test, initOptimalParamsForMinusSigmaXHamiltonian){
	...
	for(auto &ansatz_name: initial_ansatz_names){
		FastVQA::Ansatz ansatz = FastVQA::getAnsatz(ansatz_name, num_qubits, 1, 0);
...
TEST(ansatz_test, initOptimalParamsForMinusSigmaXHamiltonianDepth3){
	...
	for(auto &ansatz_name: initial_ansatz_names){
		FastVQA::Ansatz ansatz = FastVQA::getAnsatz(ansatz_name, num_qubits, 3, 0);
		FastVQA::initOptimalParamsForMinusSigmaXHamiltonian(&ansatz);
		GTEST_COUT << "Testing " << ansatz_name << std::endl;
		accelerator.run_circuit(ansatz.circuit);
...
TEST(ansatz_test, initializeMinusSigmaXHamiltonianDepth3){
